...
import {
  ...
  AlertTriangle, Snowflake, X, ClipboardList,
} from 'lucide-react'
...
export function StockOpnameClient({ role }: Props) {
  const [sessions, setSessions]     = useState<StockOpname[]>(OPNAME_LIST)
  ...
  if (view === 'list') {
    const berlangsung     = sessions.filter((s) => s.status === 'Berlangsung').length
      ...
      <>
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              { label: 'Berlangsung',      value: berlangsung, icon: Snowflake,    cls: 'text-blue-600',    glow: 'bg-blue-50'        },
                ...
                <tbody className="divide-y divide-border">
                  {sessions.map((s) => (
                    <tr key={s.id} className="bg-card transition-colors hover:bg-muted/30">
                      ...
                      <td className="px-5 py-3.5 text-xs text-muted-foreground">{formatTanggal(s.mulai)}</td>
                      <td className="px-5 py-3.5 text-muted-foreground">{s.dibuatOleh}</td>
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        >
                          {s.status === 'Berlangsung' ? 'Input' : 'Lihat'}
                        </button>
                      </td>
                    </tr>
                  ))}
  ...
  const isBerlangsung = selected.status === 'Berlangsung'
  const isMenunggu    = selected.status === 'Menunggu Approval'
  const isManager     = role === 'manager'

  // Hitung variance per item (gabungkan stokFisik dari state counts jika berlangsung)
  const itemsWithVariance = selected.items.map((item) => {
    const fisik = isBerlangsung
      ? (counts[item.productId] !== undefined ? parseInt(counts[item.productId]) || 0 : item.stokFisik)
      : item.stokFisik
    const variance = fisik !== null ? fisik - item.stokSistem : null
    return { ...item, stokFisikResolved: fisik, variance }
  })

  const totalVarianceNilai = itemsWithVariance
    .filter((i) => i.variance !== null && i.variance !== 0)
    .reduce((s, i) => s + (i.variance! * i.harga), 0)
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">{selected.area} · Dibuat oleh {selected.dibuatOleh}</p>
          </div>
          {/* Tombol Manager */}
        ...
        {/* Catatan */}
        {selected.catatan && (
          <div className="rounded-xl border border-chart-4/20 bg-chart-4/5 px-4 py-3 text-sm text-muted-foreground">
            <span className="font-medium text-chart-4">Catatan:</span> {selected.catatan}
          </div>
        )}
        ...
        <div className="rounded-xl border border-border bg-card shadow-xs">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-border">
                {itemsWithVariance.map((item) => {
                  ...
                  const nilaiSelisih = v !== null && v !== 0 ? v * item.harga : null
                  ...
                  return (
                    <tr key={item.productId} className="bg-card">
                      ...
                      <td className="px-5 py-3.5 text-right">
                        {isBerlangsung ? (
                          <Input
                            ...
                            onChange={(e) => setCounts((prev) => ({ ...prev, [item.productId]: e.target.value }))}
                      ...
                      <td className={`px-5 py-3.5 text-right tabular-nums text-xs ${nilaiSelisih !== null && nilaiSelisih < 0 ? 'text-destructive' : 'text-muted-foreground'}`}>
                        {nilaiSelisih !== null ? formatRupiah(nilaiSelisih) : '—'}
                      </td>
                      <td className="px-5 py-3.5">
                        {v === null ? (
          ...
          {/* Footer total variance */}
          {totalVarianceNilai !== 0 && (
            <div className="flex items-center justify-between border-t border-border px-5 py-3.5">
              <span className="text-xs text-muted-foreground">Total nilai variance</span>
              <span className={`text-sm font-bold tabular-nums ${totalVarianceNilai < 0 ? 'text-destructive' : 'text-chart-3'}`}>
                {totalVarianceNilai > 0 ? '+' : ''}{formatRupiah(totalVarianceNilai)}
              </span>
            </div>
          )}
        </div>
